...
Puppet::Type.type(:rabbitmq_operator_policy).provide(:rabbitmqctl, parent: Puppet::Provider::RabbitmqCli) do
  ...
  def self.policies(vhost, name)
    ...
    unless @policies[vhost]
      ...
      policy_list = run_with_retries do
        rabbitmqctl_list('operator_policies', '-p', vhost)
      end
  ...
  def destroy
    rabbitmqctl('clear_operator_policy', '-p', should_vhost, should_policy)
  end
